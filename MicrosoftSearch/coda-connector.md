...
- MET150 
- MOE150 
description: "Set up the Coda Graph Connector for Microsoft Search and Microsoft 365 Copilot" 
ms.date: 04/03/2025
---

# Coda Microsoft Graph connector (Preview)

The Coda Graph Connector allows your organization to index documents and pages from Coda. After you configure the connector, end users can search for these content from Coda in Microsoft Copilot and from any Microsoft Search client.
 
This article is for Microsoft 365 administrators or anyone who configures, runs, and monitors a Coda Graph Connector.

>[!NOTE]
>The Coda connector is in public preview. If you wish to get access to try it, you need to enable [Targeted Release](/microsoft-365/admin/manage/release-options-in-office-365#set-up-the-release-option-in-the-admin-center) ring for your Admin account.

## Capabilities
- **Indexing with Access Control**: Index documents and emails from Coda while maintaining access control.
...
## Limitations
- **Compatibility**: This Graph Connector is exclusively compatible with Coda multi-tenant Cloud (also known as Coda Work at cloudCoda.com).
- **Unsupported Environments**: Coda Work on-premises or Coda single-tenant Cloud hosted in Coda at Coda.work and other domains are not supported at this time.
- **HIPAA Compliance**: Coda content marked as requiring HIPAA compliance will not be indexed.

## Prerequisites
- You must be the **search admin** for your organization's Microsoft 365 tenant.
- **Coda instance URL**: You will need your organization's Coda instance URL, which is typically: [https://cloudCoda.com](https://cloudCoda.com/).
- **Coda NRTADMIN account**: To install the Microsoft Coda connector application in Coda, you need an Coda account with NRTADMIN permission to complete the OAuth 2.0 Authentication in Microsoft Admin Center.
- **OAuth Client_ID and Client_Secret**: Obtain the OAuth Client_ID and Client_Secret from Coda support before setting up the Coda connection in the Microsoft Admin Center.
- **Add and authorize Microsoft Coda application**: You need to add and authorize the Microsoft Coda application in the Coda Control Center.
...
### 2. Coda instance URL
The Coda instance URL is essential to correctly access and update data from, which is typically: [https://cloudCoda.com](https://cloudCoda.com/).

### 3. Authentication Type

**Coda OAuth 2.0**

We support OAuth 2.0 authentication for Coda. The connector is registered as an application titled **Microsoft - Coda Graph Connector**, which you can find on the Coda application page. To enable this application for your organization, follow these steps before setting up the Coda connection in [Microsoft 365 Admin center](https://admin.microsoft.com/Adminportal#/MicrosoftSearch/connectors).

#### Step 1: Add Microsoft Coda connector application ####
A new application registered and authorized for your Coda environment isn't enabled by default. Apps may be enabled in Coda Control Center by a user assigned to a Global Management role that has the App Management privilege.
You need to add the **Microsoft - Coda Graph Connector** in Coda Control center for your environment with below recommended values. [Learn more](https://docs.Coda.com/cloud/cc-help/en-US/Adding_an_application.html)
...
#### Step 2: Contact Coda support for OAuth credentials ####
Please create a ticket to Coda support to get your client_id and Client_secret for this Coda application to setup the Coda connection in Microsoft 365 Admin center. 
   
Enter the client ID (Unique identifier) and Secret to connect to your instance. After connecting, use a Coda NRTADMIN account credential to authenticate permission to crawl.
...
The Coda Microsoft Graph connector supports search permissions visible to **Everyone** or **Only people with access to this data source**. If you choose **Everyone**, indexed data appears in the search results for all users. If you choose **Only people with access to this data source**, indexed data appears in the search results for users who have access to them.
